...
| cluster_endpoint_public_access | Indicates whether or not the EKS public API server endpoint is enabled | bool | false | no |
| cluster_endpoint_public_access_cidrs | List of CIDR blocks which can access to the EKS cluster public API endpoint (set appropriate CIDR block(s) to restrict access as desired if cluster_endpoint_public_access is set to *true*) | list(string) | ["0.0.0.0/0"] | no |
